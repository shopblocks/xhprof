<?php
function pa($a) { echo '<pre>'.print_r($a,1).PHP_EOL.'</pre>'; }
...
if(!empty($files)) {
    ...
    $file_limit = isset($_GET['limit']) ? $_GET['limit'] : 6;
    $count = 0;
    ...
    if(!empty($file_run_ids) && !empty($_GET['source'])) {
        ...
        foreach($file_run_ids as $run_id => $file) {
            ...
            if(isset($symbol_tabs[$run_id]['main()'])) {
                ...
                $headers[] = $file_modified . '<br><a href="/xh/xhprof_html/index.php?source=shopblocks_admin&run=' . $run_id . '">'.$run_id.'</a>' . PHP_EOL;
...
<div id="menu">
    <div class="col">
        <form method="get">
            ...
            <select id="in_source" name="source" onchange="window.location='index.php?run=history&source=' + this.value;">
    ...
    <div class="col">
        <p><a href="/xh/xhprof_html/clear.php" onclick="return confirm('Are you sure?');">Clear all XHProf reports</a></p>
    </div>
</div>
